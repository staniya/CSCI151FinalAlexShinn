...
def main():
    ...
    """
    print_header('Spotify Scraper', length=50)
    ...
    while retry:
        ...
        print_header('Getting Album Cover and Genre')
        data = {}
        for track in selected_tracks:
            ...
            data[track_name] = {}
            data[track_name]['genre'] = genre_list
            data[track_name]['url'] = cover_art_url
        with open("spotifyscraper.json", "a") as write_file:
...
def list_playlists(spotify, username):
    ...
    while len(playlists) < total:
        playlists_response = spotify.user_playlists(
            username, offset=len(playlists))
        playlists.extend(playlists_response.get('items', []))
        total = playlists_response.get('total')

    # Remove any playlists that we don't own
    playlists = [playlist for playlist in playlists if playlist.get(
        'owner', {}).get('id') == username]

    # List out all of the playlists
    print_header('Your Playlists')
    for i, playlist in enumerate(playlists):
        print('  {}) {} - {}'.format(i + 1,
                                     playlist.get('name'), playlist.get('uri')))
    ...
    while len(tracks) < total:
        tracks_response = spotify.user_playlist_tracks(
            playlist_owner, playlist.get('id'), offset=len(tracks))
