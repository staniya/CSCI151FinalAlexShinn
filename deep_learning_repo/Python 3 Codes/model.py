...
class GAN:
	...
	def build_model(self):
		...
		g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(disc_fake_image), logits=disc_fake_image_logits))
		
		d_loss1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(disc_real_image), logits=disc_real_image_logits))
		d_loss2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(disc_wrong_image), logits=disc_wrong_image_logits))
		d_loss3 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(disc_fake_image), logits=disc_fake_image_logits))
	...
	def sampler(self, t_z, t_text_embedding):
		...
		z_concat = tf.concat(values=[t_z, reduced_text_embedding], axis=1)
